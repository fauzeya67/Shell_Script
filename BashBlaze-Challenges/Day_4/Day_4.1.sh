...
function view_system_metrics() {
    echo " System Metrics "
    # Fetch CPU usage using the `top` command and extract the value using awk
    cpu_usage=$(top -bn 1 | grep '%Cpu' | awk '{print $2}')
    # Fetch memory usage using the `free` command and extract the value using awk
    mem_usage=$(free | grep Mem | awk '{printf("%.1f", $3/$2 * 100)}')
    # Fetch disk space usage using the `df` command and extract the value using awk
